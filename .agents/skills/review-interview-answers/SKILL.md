...
按 `references/scoring-rubric.md` 分别评价：
...
每项都给 0–100 分、简短理由和 qNN 证据。五项各占 20%。Bridge 模式不要自行输出总分；服务器负责等权平均和四舍五入。审计或直接报告模式可展示计算值，但必须与五维平均一致。

不要为了继承旧总分而反推五维分数。若新旧总分变化，说明评分契约或证据导致的变化。
