...
export function queueRefKey(value) {
  ...
  const findy = raw.match(/findy-code\.io\/companies\/\d+\/jobs\/([^/?#]+)/i);
  if (findy) return `findy:${findy[1]}`;

  const bizreach = raw.match(/bizreach\.jp\/jobs\/(\d+)/i);
  if (bizreach) return `bizreach:${bizreach[1]}`;
