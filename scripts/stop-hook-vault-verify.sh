...
# 3 = vault-stats.mjs が「上流データを読めない」と言っている（iCloud の権限切れ等）。
# データが壊れているのではなく環境の問題なので、冒頭の方針どおりブロックはしない。
# 黙って通すと気付けないので、理由だけ見せる。
if [ "$STATUS" -eq 3 ]; then
  {
    echo "※ vault の集計はスキップされた（上流データにアクセスできない）。台帳は書き換えていない。"
    echo "$OUTPUT"
  } >&2
  exit 0
fi
