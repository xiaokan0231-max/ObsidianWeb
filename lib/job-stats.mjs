 ...
 * 凍結済みの CSV は iCloud にあり Web からは到達不能で、集計を持っているのは
 ...
 *
 * 🔴 **窓の中だけで完結させる。** 窓より前の過去の不採用をこの引き算に混ぜてはいけない——
 ...
 * （例：メールは「ＮＴＸジャパンホールディングス株式会社」、vault は
 * 「NTXソフトウェアジャパン株式会社」）。空なら会社名をそのまま使う。
 *
 ...
 * 分母が一桁の経路は、1社動くだけで率が数十ポイント跳ねる。
 * 分母が三桁ある経路の率と同じ画面に並べると、少数サンプルだけが過大に見える。
