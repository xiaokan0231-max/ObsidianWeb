...
export function planVaultCompaction(notes, { now = new Date(), reportRetentionDays = 90 } = {}) {
  ...
  for (const note of notes) {
    ...
    // 契合评价保留历次判断，schema v1 与通用分析归档 v2 不是同一套协议。
    // 即使已被正本的新指针替代，也不能由通用保留期限改写或移走历史。
    if (reportType === "ai-report" &&
      String(note.frontmatter.report_kind ?? "").replace(/^["']|["']$/g, "") === "company-fit") {
      keep.push({ path: note.relativePath, reason: "公司契合评价及其历史版本" });
      continue;
    }
