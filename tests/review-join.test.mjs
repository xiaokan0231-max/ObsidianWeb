...
test("会社名の表記揺れ（法人格・全角・略記）でも同じ回として束ねる", () => {
  // 実データで起きた形：整理稿は「株式会社ＫＲＴ」、批注はディレクトリ名由来の「KRT」。
  // 厳密一致だとこの批注が復盤画面から静かに消える（実際に消えていた）。
  const seirikou = note(
    "20_求職/KRT/2026-08-10_カジュアル面談_整理稿.md",
    { type: "transcript-study", company: "株式会社ＫＲＴ", date: "2026-08-10", round: "カジュアル面談" },
  );
  const annotation = note(
    "20_求職/KRT/2026-08-10_カジュアル面談_批注.md",
    { type: "study-annotation", company: "KRT", date: "2026-08-10" },
  );
  ...
  const other = note(
    "20_求職/KRX/2026-08-10_カジュアル面談_批注.md",
    { type: "study-annotation", company: "株式会社KRX", date: "2026-08-10" },
  );
